...
public:
	...
	OpenFrp(string exePath, string Arch, string SubPlatform = "Linux") {
	...
	void welcome() {
		system("mkdir ~/.OpenFrp");
		printf("Welcome! OpenFrp.%s (OpenFrp.Unix Core Version %s %s)\n", SubPlatform.c_str(), Version, Arch.c_str());
		printf("\n");
		printf("  * Website:   https://www.openfrp.net \n");
		printf("  * GitHub:    https://github.com/OldGodShen/OpenFrp \n");
		printf("  * 仅限非商业使用 \n");
		printf("\n");
	...
	void lsfrps() {
		system(("curl --location --request POST 'https://api.openfrp.net'  --header 'MarchOpenFrpLauncher' --form 'action=getnodestext' --form 'apitoken=LTW9yDKEfYJnzrqv' --form 'token=" + token + "'").c_str());
		printf("\n");
	}
	...
	void startfrpc(string nodeid, string actoken) {
		...
		string iniPath = "~/.OpenFrp/frpc.ini";
		string curlsh = "curl --location --request POST 'https://api.openfrp.net' --header 'MarchOpenFrpLauncher' --form 'action=getconf' --form 'token=" + token + "' --form 'node=" + nodeid + "' -o " + iniPath;
		system(curlsh.c_str());
	...
	void printHelp() {
		...
		printf("token [token]      Set your OpenFrp access token. | token: your OpenFrp access token\n");
	}
