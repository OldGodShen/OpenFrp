...
namespace OpenFrp.UI
{
    public class Frp
    {
        ...
        string path = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\OpenFrp";
        ...
        public int reDownloadFrpc()
        {
            try
            {
                Net.Download("https://client.ogfrp./frpc/frpc_windows_amd64.exe", path + "\\frpc.exe", true);
            }
            catch (Exception)
            {
                return -1;
        ...
        /// <param name="token">OpenFrp用户token</param>
        /// <returns></returns>
        public int serverToId(string server, string token)
        {
            FrpServer frpServer = new FrpServer();
            FrpServerModel[] frpServers = frpServer.FrpsServerList(Net.Get("https://api.openfrp.net/?apitoken=LTW9yDKEfYJnzrqv&action=getnodesidip&token=" + token)).ToArray();
        ...
        /// </summary>
        /// <param name="token">OpenFrp用户token</param>
        /// <param name="server">OpenFrp的节点ID</param>
        /// <returns></returns>
        public int launchFrpc(string token, int server)
        {
            try
            {
                File.WriteAllText(Config.FolderPath + "\\frpc.ini", Net.Get("https://api.openfrp.net/?action=getconf&token=" + token + "&node=" + server.ToString()));
        ...
        /// </summary>
        /// <param name="token">OpenFrp用户token</param>
        /// <param name="server">OpenFrp节点IP</param>
        /// <returns></returns>
        public int launchFrpc(string token, string server)
        {
            try
            {
                ...
                File.WriteAllText(Config.FolderPath + "\\frpc.ini", Net.Post("https://api.openfrp.net/", frpciniurl));
                Interaction.Shell(Environment.CurrentDirectory + "\\frpc.exe -c \"" + Config.FolderPath + "\\frpc.ini\"", AppWinStyle.NormalFocus);
            }
            catch (Exception ex)
            {
                MsgBox("Failure at\n" + ex.ToString(), MsgBoxStyle.Critical, "OpenFrp");
                return -1;
            }
            return 0;
        }
